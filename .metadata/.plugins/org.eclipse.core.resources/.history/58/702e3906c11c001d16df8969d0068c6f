...
public class DonViTinh_view extends JFrame {
	...
	public DonViTinh_view() {
		...
		txt_timKiem.setBounds(10, 18, 1025, 29);
	...
	public void updateTable() {
		...
		int stt = 1;
		List<DonViTinhModel> listDVTM = new ArrayList<DonViTinhModel>();
		...
		if(keySearch.equals("") || keySearch.equals("Nhập tên đơn vị muốn tìm")) {
			for (DonViTinhModel donViTinhModel : listDVTM) {
				model.addRow(new Object[] {stt,donViTinhModel.getMaDonVi(),donViTinhModel.getTenDonVi(),donViTinhModel.getTrangThai()});
				stt++;
			}
		}else {
			for (DonViTinhModel donViTinhModel : listDVTM) {
				if(donViTinhModel.getTenDonVi().toLowerCase().indexOf(keySearch.toLowerCase())>=0) {
					model.addRow(new Object[] {stt,donViTinhModel.getMaDonVi(),donViTinhModel.getTenDonVi(),donViTinhModel.getTrangThai()});
					stt++;
				}
			}
